...
def reproject_with_depth_K(coord,depth_ref, intrinsics_ref, extrinsics_ref):
    # 假设 depth_ref 的 shape 是 (B, C, N)，你要获取 N
    ...
    # 检查是否为完全平方数
    assert side * side == n_pixels, f"depth_ref.shape[2] ({n_pixels}) 不是一个完全平方数"
    ...
    intrinsics_ref[:, 0, :] *= float(w) / 256.0  # 调整 fx 和 cx
    intrinsics_ref[:, 1, :] *= float(h) / 256.0  # 调整 fy 和 cy
    ...
    # # # 确保输入为 float64 类型
    # depth0 = load_pfm("/run/user/1000/gvfs/smb-share:server=10.101.190.11,share=huangxuejun/dataset/MV_SSeg/US3D_RPC/SAT_3DGS_new/CAM_CROP256_WHU_TLC_DFC19_dataset/train/depth/0/JAX_Tile_113_RGB_004_crop_0.pfm")
    # depth1 = load_pfm("/run/user/1000/gvfs/smb-share:server=10.101.190.11,share=huangxuejun/dataset/MV_SSeg/US3D_RPC/SAT_3DGS_new/CAM_CROP256_WHU_TLC_DFC19_dataset/train/depth/1/JAX_Tile_113_RGB_015_crop_0.pfm")
    # depth2 = load_pfm("/run/user/1000/gvfs/smb-share:server=10.101.190.11,share=huangxuejun/dataset/MV_SSeg/US3D_RPC/SAT_3DGS_new/CAM_CROP256_WHU_TLC_DFC19_dataset/train/depth/2/JAX_Tile_113_RGB_019_crop_0.pfm")
    # depth0 = load_pfm("/run/user/1000/gvfs/smb-share:server=10.101.190.11,share=huangxuejun/dataset/MV_SSeg/US3D_RPC/SAT_3DGS_new/CAM_CROP256_WHU_TLC_DFC19_dataset/test/depth/0/JAX_Tile_017_RGB_004_crop_43.pfm")
    # depth1 = load_pfm("/run/user/1000/gvfs/smb-share:server=10.101.190.11,share=huangxuejun/dataset/MV_SSeg/US3D_RPC/SAT_3DGS_new/CAM_CROP256_WHU_TLC_DFC19_dataset/test/depth/1/JAX_Tile_017_RGB_015_crop_43.pfm")
    # depth2 = load_pfm("/run/user/1000/gvfs/smb-share:server=10.101.190.11,share=huangxuejun/dataset/MV_SSeg/US3D_RPC/SAT_3DGS_new/CAM_CROP256_WHU_TLC_DFC19_dataset/test/depth/2/JAX_Tile_017_RGB_019_crop_43.pfm")
    # depth0 = depth0.astype(depth0.dtype.newbyteorder('='))
    # depth0 = torch.from_numpy(depth0)
    # depth1 = depth1.astype(depth1.dtype.newbyteorder('='))
    # depth1 = torch.from_numpy(depth1)
    # depth2 = depth2.astype(depth2.dtype.newbyteorder('='))
    # depth2 = torch.from_numpy(depth2)
    # depth = torch.cat([depth0, depth1, depth2], dim=0).reshape(3, -1)
    # depth_ref = depth.to(dtype=torch.float64, device=device)
    ...
    row = coord[...,1].reshape(b*v, h,w) * w # coord考虑了由深度学习网络，学习而来的偏移。为3DGS提供更多冗余。且在encoder_costvolume中考虑了0.5，即表示光线从中心点出发（3DGS从中心点投影到ENU坐标系下）

    row_test = (torch.arange(h, dtype=torch.float64, device=device) + 0.5).view(h, 1).expand(b * v, h, w)
    col_test = (torch.arange(w, dtype=torch.float64, device=device) + 0.5).view(1, w).expand(b * v, h, w) # 表示光线从中心点出发
    # row = torch.arange(h, dtype=torch.float64, device=device).view(h, 1).expand(b*v, h, w)
    # col = torch.arange(w, dtype=torch.float64, device=device).view(1, w).expand(b*v, h, w)
    # 展平坐标
...
def sample_image_grid(
    ...
    # coordinates = [(idx + 0.5) / length for idx, length in zip(indices, shape)] # 不应该加0.5
    coordinates = [idx / (length - 1) for idx, length in zip(indices, shape)] # 不应该加0.5
