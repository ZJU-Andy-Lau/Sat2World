...
class DatasetRE10k(Dataset):
    ...
    def __init__(self, cfg: DatasetRE10kCfg, stage: Stage):
        ...
        for root in cfg.roots:
            ...
            root = Path(root) / "test2048" / "image/0"
    ...
    def load_one_chunk(self, chunk_path: Path):
        ...
        JAX_index = ["RGB_002", "RGB_015", "RGB_016"]
        OMA_index = ["RGB_034", "RGB_039", "RGB_041"]
        ATL_index = ["RGB_007", "RGB_025", "RGB_032"]
        ...
        # print(filename)
        # load SAM / CLIP / gt height
        gt_height_list, height_DAM3_list, height_pseudoLabel_list = [], [], []
        for fn in filename:
            # gt height
            ...
            if self.stage == "train":
                ...
                path = fn.replace("/image/", "/height_pseudoLabel/").replace("_RGB_", "_XYZ_")
                height_pseudoLabel = torch.from_numpy(tifffile.imread(path)).float()
                height_pseudoLabel_list.append(height_pseudoLabel)

        results["context"]["gt_height"]  =  np.stack(gt_height_list)

        if self.stage == "train":
            results["context"]["height"] = np.stack(height_DAM3_list)
            results["context"]["height_pseudoLabel"] = np.stack(height_pseudoLabel_list)
        else:
            # --------------------------------------------
            # 自动发现 target 视角文件 (/3/, /4/) —— 无固定 index
            # --------------------------------------------
            target_filename = []
            ...
            for idx in [3, 4]:
                ...
                # 筛选：tile_prefix 一致 + crop_suffix 一致
