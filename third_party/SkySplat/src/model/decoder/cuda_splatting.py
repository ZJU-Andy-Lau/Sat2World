...
def get_fov(intrinsics: Float[Tensor, "batch 3 3"]) -> Float[Tensor, "batch 2"]:
    intrinsics[..., 0, :] *= 1 / 256.0  # 使用单位内参（与原始代码保持一致）
    intrinsics[..., 1, :] *= 1 / 256.0  # 使用单位内参（与原始代码保持一致）
...
def render_cuda(
    ...
    for i in range(b):
        ...
        ### save
        # per_filename =  filename[i][0].split('/')[-1].split('.tif')[0]
        ...
        # img = (rendered_image.clamp(0, 1) * 255).byte().permute(1, 2, 0).cpu().numpy()
