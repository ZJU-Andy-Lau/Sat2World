...
def rotmat_to_quat(rot_matrices):
    ...
    for rot in rot_matrices:
        ...
        per_quats = Rotation.from_matrix(rot).as_quat()  # 直接转换，[x,y,z,w]顺序
        scipy_quat = per_quats[[3, 0, 1, 2]]  # 将scipy的四元数转换为[w,x,y,z]顺序
...
# def rotmat_to_quat(rot_matrices):
#     inputs = rot_matrices
#     rot_matrices = rot_matrices.cpu().numpy()  # 转换为 NumPy
#     quats = []
#     for rot in rot_matrices:
#         per_quats = Rotation.from_matrix(rot).as_quat()  # 直接转换
...
class GaussianAdapter(nn.Module):
    ...
    def forward(
        ...
        scales, rotations, sh = raw_gaussians.split((3, 4, 3 * self.d_sh), dim=-1) # XYZ_scales; 四元数rotations; 球谐系数，用于颜色编码。
        ...
        rotations = rotations / (rotations.norm(dim=-1, keepdim=True) + eps) # rotations 的 L2 范数 并归一化，确保其是 单位四元数
        # Apply sigmoid to get valid colors.
        sh_coeffs = rearrange(sh, "... (xyz d_sh) -> ... xyz d_sh", xyz=3)

        # sh_coeffs: (B, V, N, 1, 1, 3, 25)
        colors = data_samples['image']
        B, V, N, _, _, _, D = sh_coeffs.shape
        sh_dc = sh_coeffs[..., 0]  # (B, V, N, 1, 1, 3)
        sh_rest = sh_coeffs[..., 1:]  # (B, V, N, 1, 1, 3, 24)
        # 对 colors 插值到 (img_h, img_w)
        # colors: (B, V, 3, H, W) -> (B*V, 3, H, W)
        colors_ = colors.reshape(B * V, 3, *colors.shape[-2:])
        colors_ = F.interpolate(colors_, size=(img_h, img_w), mode="bilinear", align_corners=False,)
        colors_ = colors_.reshape(B, V, 3, img_h, img_w)
        # RGB → SH(DC)
        sh_rgb = RGB2SH(colors_)  # (B, V, 3, img_h, img_w)
        # (B, V, 3, 1, 1) -> (B, V, N, 1, 1, 3)
        sh_rgb = (sh_rgb.permute(0, 1, 3, 4, 2).reshape(B, V, N, 1, 1, -1))
        # DC 修正
        sh_dc = sh_dc + sh_rgb  # (B, V, N, 1, 1, 3)
        sh = torch.cat([sh_dc.unsqueeze(-1), sh_rest], dim=-1)  # (B, V, N, 1, 1, 3, 25)
        sh = sh.broadcast_to((*opacities.shape, 3, self.d_sh)) * self.sh_mask # 广播 sh 使其匹配 opacities 形状


        # Create world-space covariance matrices.
        covariances = build_covariance(scales, rotations) # 相机坐标系下的covariances。这里的rotations是高斯头预测出来的，不等于真正的协方差公式。和81行不一样
        c2w_rotations = extrinsics[..., :3, :3]
        covariances = c2w_rotations @ covariances @ c2w_rotations.transpose(-1, -2) # 通过外参旋转，将协方差矩阵从 相机空间 转换到 世界空间。这一步才是对应于协方差获得公式中的代码
        covariances = rearrange(covariances, "b v i () () m n -> b v i m n")

        # Compute Gaussian means.
        ### 换成RPC的
        x_rpc = coordinates[...,0].reshape(batch*view, -1) * img_w # 由于下面3DGS中心点mean3d坐标
        y_rpc = coordinates[...,1].reshape(batch*view, -1) * img_h ### 注意,RPC_Photo2Obj是基于整数像素坐标,不需要归一化!
        x_rpc, y_rpc = x_rpc.clamp(0,img_w), y_rpc.clamp(0,img_h)
        height = depths.reshape(batch*view, -1)
        ### 缩放RPC (*64/256)
        rpc_proj_matrices[..., 0] = rpc_proj_matrices[..., 0] * float(img_w) / 256.0
        rpc_proj_matrices[..., 1] = rpc_proj_matrices[..., 1] * float(img_h) / 256.0
        rpc_proj_matrices[..., 5] = rpc_proj_matrices[..., 5] * float(img_w) / 256.0
        rpc_proj_matrices[..., 6] = rpc_proj_matrices[..., 6] * float(img_h) / 256.0
        ...
        img_path = data_samples['ref_filename'][0][0]
        flag_city = img_path.split('/')[-1].split('_')[0]
        ...
        fixed_rotations,flag_Not_ortho = rotmat_to_quat(extrinsics_to_quat_input) # (仅训练时需要)

        fixed_rotations = fixed_rotations.unsqueeze(2).expand(-1, -1, means3d.size(2), -1) # means.size(2)表示像方的大小,这里是256*256
        ...
        return Gaussians(
            ...
            harmonics=rotate_rgb_harmonics.to(dtype=torch.float), # 将球谐旋转到世界空间。(仅训练时需要)
            opacities=opacities.to(dtype=torch.float),
            scales=fixed_scales.to(dtype=torch.float),
            rotations=fixed_rotations.to(dtype=torch.float), # (仅训练时需要)
        )
