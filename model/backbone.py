...
2) GeometryTokenMLP: 把 45D patch 几何特征编码到 256D；
...
class GeometryTokenMLP(nn.Module):
    """几何 token 编码器：45D -> 256D。

    结构:
        Linear(45->hidden) + GELU + LayerNorm + Linear(hidden->256) + LayerNorm。
    """

    def __init__(self, in_dim: int = 45, hidden_dim: int = 256, out_dim: int = 256) -> None:
        """初始化几何特征 MLP。"""
        super().__init__()
