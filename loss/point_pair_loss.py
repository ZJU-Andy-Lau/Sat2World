...
class PointReprojectionLoss:
    ...
    定义：
    - 对每个视图 i，选择 j=(i+1)%V；
    - 将 view_i 的预测点云 point_abs(i) 投影到 view_j，得到 point_proj；
    - 使用 rpc_gt + h_gt 从 view_i 像素网格构造对应的 point_gt(在 view_j 像平面)；
    - 以两者像素距离作为重投影误差。
    """
    ...
    def __call__(self, point_abs: torch.Tensor, batch: dict[str, Any]) -> tuple[torch.Tensor, dict[str, torch.Tensor], dict[str, Any]]:
        b, v, _, h, w = point_abs.shape
        device = point_abs.device
        dtype = point_abs.dtype
        if v < 2:
            zero = torch.zeros((), device=device, dtype=dtype)
            return zero, {"point_reproj_px_mean": zero, "point_reproj_num_pairs_used": zero}, {"point_reproj_num_pairs_used": 0}
        ...
        for bi in range(b):
            for i in range(v):
                ...
                # 预测点云(view_i) -> view_j
                p_i, in_p = sample_map_bilinear(point_abs[bi : bi + 1, i], src_pts)
                if not bool(in_p.any()):
                    continue
                p_i = p_i[0].transpose(0, 1)  # [N,3]
                xs_pred = p_i[:, 0].view(1, 1, -1)
                ys_pred = p_i[:, 1].view(1, 1, -1)
                h_pred = p_i[:, 2].view(1, 1, -1)
