...
class BackbonePyramid(torch.nn.Module):
    ...
    def extract_feature(self, context):
        ...
        out_feature = self.backbone(imgs)
        ...
        p2 = p2.view(*p3.shape[:2], 64, 128, 128)
        # ===== P1 =====
        p1 = self.fpn_p2_to_p1(p2.flatten(0, 1))
        p1 = p1.view(*p3.shape[:2], 32, 256, 256)
        multi_scale_features = [p3, p2, p1]
        return (out_feature, multi_scale_features)
...
class DinoExtractor(torch.nn.Module):
    def __init__(self, vit_path="/data/huangxuejun/project/SkySplat++_Lightning/SkySplat++_guidedHeight/checkpoints/dinov2_vitb14_pretrain.pth"):
