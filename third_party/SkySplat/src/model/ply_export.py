...
def color2feat(color):
    ...
    color = (color - 0.5) / 0.28209479177387814  # 将RGB映射为球谐DC分量
    features_dc = color[:, :, 0:1]          # SH 0阶（第二个维度3表示RGB）
    return features_dc
...
def save_ply(scene, path):
    # mask = scene.mask.detach().cpu().numpy() # 是否选择后处理C.A. mask
    # xyz = torch.cat([gf.reshape(-1, 3) for gf in scene.means[0]], dim=0).detach().cpu().numpy() # 若不需拼接，保留这一行（ENU坐标）
    xyz = torch.cat([gf.reshape(-1, 3) for gf in scene.utmmean[0]], dim=0).detach().cpu().numpy() # 若不需拼接，注释这一行即可（UTM坐标）
    xyz[:, :2] *= -1  # 仅对 x 和 y 取相反数，实现绕 Z 轴旋转 180 度
    ...
    # xyz,scale,opacities,rotation,rgb = xyz[mask],scale[mask],opacities[mask],rotation[mask],rgb[mask] ### 若不使用C.A.，则注释
    ...
    opacity_thresh = 0.01
    scale_thresh = 1.2
    mask = (opacity_mean > opacity_thresh) & (scale_norm < scale_thresh) # 筛选掉一些不透明度太小，scale太大的点
